...
const EMPTY_MODIFIER_STATE: ShortcutModifierState = {
  metaKey: false,
  ctrlKey: false,
  altKey: false,
  shiftKey: false,
...
/**
 * Tracks bare modifier presses in capture phase so shortcut hints appear
 * before a digit is pressed and reset reliably if the app loses focus.
 */
export function useShortcutModifierState(): ShortcutModifierState {
  const [state, setState] = useState(EMPTY_MODIFIER_STATE);

  useEffect(() => {
    const onKeyboardEvent = (event: KeyboardEvent) => {
      setState((current) => {
        const modifier = modifierStateKey(event.key);
        const next = modifier
        ...
        return modifierStatesEqual(current, next) ? current : next;
      });
    };
    const reset = () => {
      setState((current) =>
        modifierStatesEqual(current, EMPTY_MODIFIER_STATE)
          ? current
...
function modifierStateKey(key: string): keyof ShortcutModifierState | null {
  if (key === "Meta" || key === "OS" || key === "Command") return "metaKey";
  if (key === "Control") return "ctrlKey";
  if (key === "Alt" || key === "Option") return "altKey";
  if (key === "Shift") return "shiftKey";
  return null;
}

/** True when the held modifiers exactly match a binding, ignoring its key. */
export function matchesKeybindingModifiers(
  ...
  if (
    !parts.some(
      (part) =>
        part === "mod" || part === "ctrl" || part === "meta" || part === "alt",
    )
  ) {
    return false;
  }
  const mod = parts.includes("mod");
  return (
    modifiers.metaKey === (parts.includes("meta") || (mod && isMac())) &&
    modifiers.ctrlKey === (parts.includes("ctrl") || (mod && !isMac())) &&
    modifiers.altKey === parts.includes("alt") &&
    modifiers.shiftKey === parts.includes("shift")
  );
}
